(function () {
  ...
  function polar(radius, angle) {
    const a = (angle - 90) * Math.PI / 180; // 0° at top
  ...
  function init(root) {
    const src = root.dataset.src || '/js/attention/art-elements-wheel.json';
    fetch(src).then(r => r.json()).then(data => {
      ...
      function currentIndex() {
        // index whose mid-angle is nearest to 0° (top)
      ...
      // Button controls (animate one step)
  ...
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.attention-wheel').forEach(init);
  });
})();
